...
@section('content')
    <div class="grid gap-6 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <p class="text-xs uppercase tracking-wide text-slate-500">Categories</p>
    ...
    <div class="grid gap-6 lg:grid-cols-[1.2fr,0.8fr]">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between">
                ...
                <span class="text-sm text-slate-500">Last {{ $recentOrders->count() }} records</span>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700">
                        @forelse ($recentOrders as $order)
                            <tr>
                                ...
                                <td class="py-3">{{ $order->customer_name ?? $order->user?->name ?? 'Guest' }}</td>
                                <td class="py-3">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-6 text-center text-slate-500">No orders yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                @forelse ($bestSellers as $product)
                    <li class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-500">Sold {{ (int) ($product->sold_quantity ?? 0) }} units</p>
                        </div>
                    </li>
                @empty
                    <li class="text-sm text-slate-500">No product sales yet.</li>
                @endforelse
            </ul>
        </div>
    </div>
