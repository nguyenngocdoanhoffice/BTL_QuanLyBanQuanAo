@php
    ...
    $image = $product->cover_image ? Storage::url($product->cover_image) : 'https://placehold.co/600x800?text=DOCHA+Thoi+trang';
    ...
    $previewPayload = [
        'id' => $product->id,
        'title' => $product->title,
        'brand' => $product->brand?->name ?? 'DOCHA',
...
<div class="group relative flex flex-col rounded-2xl border border-slate-100 bg-white shadow-sm transition hover:-translate-y-1">
    ...
    <div class="flex flex-1 flex-col gap-2 p-4">
        ...
        <p class="text-sm text-slate-500">{{ $product->brand?->name ?? 'DOCHA' }}</p>
